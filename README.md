...
**v2.2.1 (Agent-Enhanced Edition)**
**日期**: 2026-07-11
**状态**: 已通过语法、ShellCheck 和 Android Binder 命令实机验证
...
| `/vendor_dlkm` 等 | bind ro | 设备存在时映射动态内核模块与厂商扩展分区 |
...
| `/vendor_dlkm /system_dlkm /odm_dlkm` | 同名 | bind | ro | 动态内核模块分区（存在时） |
| `/oem /cust /my_*` | 同名 | bind | ro | 常见厂商扩展分区（存在时） |
...
### v2.2.1 (2026-07-11) — 当前版本
- 修复登录初始化或 Agent 重写 `PATH` 后 `am` 找不到内部 `cmd` 的问题
- 持久化 Android 运行环境，并为常用 Binder/系统命令安装受管包装器
- 补齐动态内核模块与常见厂商扩展分区的只读视图
- 增加 Android 运行时回归测试与 ShellCheck 检查
